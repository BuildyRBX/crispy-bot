...
module.exports = {
	id: 'softban',
	desc: 'Bans the specified user then unbans them deleting all of their messages in the last 7 days.',
	channels: 'guild',
	exec: async (call) => {
		...
		if (!user)
			return call.message.channel.send(`Please specify a user to softban. e.g. \`${call.client.prefix}softban ${call.client.owner.tag} For bullying me!\``);

		user = call.message.guild.member((user.match(/\d+/) || [])[0]) || await call.client.fetchUser(user).catch(() => null);

		if (!user)
			return call.message.channel.send(`Please specify a valid user to softban. You must either mention them or supply their ID. e.g. \`${call.client.prefix}softban ${call.client.owner.tag} For bullying me!\``);
		...
		if (user instanceof GuildMember && (user.highestRole.position >= call.message.member.highestRole.position || user.id === call.message.guild.ownerID))
			return call.message.channel.send('You do not have permission to softban this user.');
			...
			.then(() => {
				call.client.logChannel.send(
					new RichEmbed()
						...
						.addField('User Softbanned', user.toString())
						.addField('Softbanned By', call.message.author.toString())
						.addField('Reason', reason)
				).catch(() => {});

				infractions.addInfraction({ type: 'softban', committer: call.message.author.id, reason });

				call.message.channel.send(`Successfully softbanned ${user.tag}`);
			})
			.catch(() => call.message.channel.send(`Failed to softban ${user.tag}.`));
	}
};